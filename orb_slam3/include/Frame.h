...
public:
    ...
    Frame(const cv::Mat &imMask, const cv::Mat &imGray, const double &timeStamp, ORBextractor* extractor,ORBVocabulary* voc, GeometricCamera* pCamera, cv::Mat &distCoef, const float &bf, const float &thDepth, Frame* pPrevF = static_cast<Frame*>(NULL), const IMU::Calib &ImuCalib = IMU::Calib());
    ...
    void RemoveMaskedKeypoints(const cv::Mat& imMask, std::vector<cv::KeyPoint> &mvKeys, cv::Mat &mDescriptors);
